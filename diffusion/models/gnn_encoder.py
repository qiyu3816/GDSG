...
class GNNLayer(nn.Module):
  ...
  def forward(self, h, e, graph, edge_matrix=None, mode="residual", edge_index=None, real_edge_index=None, sparse=False):
    """
    Args:
        In Dense version:
          ...
          edge_matrix: Real edge 0-1 indices (B x V x V) for dense mode.
          mode: str
        In Sparse version:
          ...
          real_edge_index: Real edge indices vector (E) for sparse mode.
    ...
    # Do a forced gating to set non-existent edge gates to 0, making it more generalizable to graphs of any sparsity
    if not sparse:
      gates = torch.sigmoid(e) * edge_matrix.unsqueeze(-1)  # B x V x V x H / E x H
    else:
      gates = torch.sigmoid(e) * real_edge_index.unsqueeze(-1)  # E x H = (B x U x S) x H
...
class GNNEncoder(nn.Module):
  ...
  def __init__(self, n_layers, hidden_dim, out_channels=1, aggregation="sum", norm="layer",
    ...
    edge_dim = 8 if out_channels == 3 else 7
  ...
  def dense_forward(self, x, graph, timesteps, adj_matrix):
    """
    Args:
    ...
    graph = torch.cat((graph, adj_matrix), dim=1).permute((0, 2, 3, 1))  # (B x V x V x [data_dim + attr_dim])
  ...
  def sparse_forward(self, x, graph, timesteps, edge_index):
    ...
    Args:
        ...
        graph: Graph edge features (E x [data_dim])
    ...
    if graph.dim() != 2:
      graph = graph.unsqueeze(-1)
    graph = torch.cat((graph, edge_index.T[:, 2:]), dim=1)
    e = self.edge_embed(self.edge_attr_embed(graph))
    time_emb = self.time_embed(timestep_embedding(timesteps, self.hidden_dim))
    all_edge_index = edge_index[:2].long()
    real_edge_index = edge_index[2].long()

    x, e = self.sparse_encoding(x, e, all_edge_index, real_edge_index, edge_index[2:], time_emb)
  ...
  def sparse_encoding(self, x, e, edge_index, real_edge_index, edge_attr, time_emb):
  ...
  def custom_freeze(self):
    ...
    self.freeze_one_layer(self.time_embed)
    for l1, l2, l3 in zip(self.layers, self.time_embed_layers, self.per_layer_out):
      self.freeze_one_layer(l1)
      self.freeze_one_layer(l2)
      self.freeze_one_layer(l3)
