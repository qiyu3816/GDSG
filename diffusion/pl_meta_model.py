...
class COMetaModel(pl.LightningModule):
    def __init__(self,
        ...
        if self.diffusion_type == 'gaussian':
            ...
            out_channels = 3
            self.gau_diffusion = GaussianDiffusion(
                T=self.diffusion_steps, schedule=self.diffusion_schedule)
            self.cat_diffusion = CategoricalDiffusion(
                T=self.diffusion_steps, schedule=self.diffusion_schedule)
        else:
    ...
    def edge2adj(self, edge_index, batch_size, server_num, user_num, device):
        ...
        Only for msco.
        """
        adj = torch.zeros((edge_index.shape[0] - 2, server_num + user_num, server_num + user_num), device=device)
        edge_num = server_num * user_num
        edge_index_long = edge_index[:3].long()
        for j in range(edge_num):
            if edge_index_long[2, j] == 1:
                adj[:, edge_index_long[0, j], edge_index_long[1, j]] = edge_index[2:, j]
        adj = adj.unsqueeze(0).repeat(batch_size, 1, 1, 1)
        return adj
    ...
    def train_dataloader(self):
        ...
        if self.args.alternate_step > 0:
            train_loaders = {}
            train_loaders['target_d'] = GraphDataLoader(self.train_dataset, batch_size=batch_size, shuffle=True,
                                                        num_workers=self.args.num_workers, pin_memory=True,
                                                        persistent_workers=True, drop_last=True)
            train_loaders['alternate_d'] = GraphDataLoader(self.alternate_dataset, batch_size=batch_size, shuffle=True,
                                                      num_workers=self.args.num_workers, pin_memory=True,
                                                      persistent_workers=True, drop_last=True)
